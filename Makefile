...
IMAGE      ?= chapkit-minimalist-example-py:latest
GHCR_IMAGE ?= ghcr.io/chap-models/chapkit_minimalist_example_py:latest
TEST_NAME  ?= chapkit-minimalist-example-py-test
TEST_PORT  ?= 9000
...
run: build
	...
	@docker run --rm -p 8000:8000 --name chapkit-minimalist-example-py $(IMAGE)

run-ghcr:
	@echo ">>> Running $(GHCR_IMAGE) on :8000"
	@docker run --rm --pull always --platform linux/amd64 -p 8000:8000 --name chapkit-minimalist-example-py $(GHCR_IMAGE)
